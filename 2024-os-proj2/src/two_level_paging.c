...
void initialize_free_page_list() {
    ...
    // RAM의 3/4를 선점 (1GB만 사용 가능)
    for (int i = 0; i < 3 * TOTAL_PAGE_SIZE / 4; i++) {
        free_page_list[i] = false; // 3/4 프레임 선점
    }
}
...
int main() {
    fp = fopen("two_level_paging_dump.txt", "w");
    if (fp == NULL) {
